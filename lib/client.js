...
class Client {
  constructor (socket) {
    ...
    socket.on('data', data => {
      ...
      if (dataAsObject.command == '__net__fin') {
        this.disconnectionCallbacks.forEach(fn => fn());
        this.send('__net__fin__ack', {});
        this.disconnect();
      }
  ...
  /**
   * Registers an event handler for when the socket connection is gracefully closing
   * @param  {function} handler
   */
  onDisconnect (handler) {
    this.disconnectionCallbacks.push(handler);
  }
